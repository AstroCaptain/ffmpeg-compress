...
# Check arguments
if [[ $# -lt 2 ]]; then
    echo "Usage: $0 [-n|--dry-run] <file1> [file2 ...] <target_size_MB>"
    exit 1
fi
...
for INPUT in "${FILES[@]}"; do
    ...
    output_file="${INPUT%.*}-$target_size_mb""MB.mp4"
    ...
    if [[ $dryrun -eq 1 ]]; then
        ...
        echo "  Audio bitrate: ${T_ARATE}k"
        echo "  Video bitrate: ${video_bitrate_kbps}k"
        echo
        continue
    fi
